...
async function mintTokensPoly(receipient, amount) {

    noncePoly = await web3UpdatePoly.eth.getTransactionCount(owner);



    tokenPolyUpdate.methods
...
async function startProcess() {
    ...
    tokenEthUpdate = new web3UpdateEth.eth.Contract(AUDITTOKEN["abi"], auditTokenAddressEth);


    console.log("Bridge Activeted....")


    tokenEth.events.Transfer({ fromBlock: 'latest' })
        .on('data', async function (event) {
            ...
            if (event.returnValues.from != addressZero)
                await mintTokensPoly(from, amount);
